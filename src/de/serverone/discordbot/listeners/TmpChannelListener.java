...
public class TmpChannelListener extends ListenerAdapter {
	...
	if (!event.getChannel().getId()
		.equals(ServerOneConfig.getConfig(ServerOneDiscordBot.getPlugin(), "bot-settings.yml")
			.getString("ids.channels.tmp-channel-creator")))
	    return;
	...
	if (member.getVoiceState().getChannel() == null
		|| !member.getVoiceState().getChannel().getName().equals("get-started")) {
	...
	if (!channel.getParent().getName().equals("tmp"))
	    return;
	if (channel.getName().equals("get-started"))
	    return;
